...
function validateProvenance(
  ...
  if (!isRecord(value)) {
    issues.push(
      issue("error", "INVALID_PROVENANCE", "Expected an object", path),
    );
    return false;
  }
  ...
  if (typeof value.status !== "string" || !statuses.has(value.status)) {
    issues.push(
      issue(
        ...
        "Provenance status must be declared, observed, generated, or inferred",
        `${path}.status`,
      ),
    );
  }
  if (typeof value.assertedBy !== "string" || value.assertedBy.length === 0) {
    issues.push(
      issue(
        "error",
        "INVALID_ASSERTER",
        "Provenance assertedBy must be a non-empty string",
        `${path}.assertedBy`,
      ),
    );
  }
...
function validateStringArray(
  ...
  path: string,
  issues: ValidationIssue[],
  options: { nonEmpty?: boolean; identifiers?: boolean } = {},
): value is string[] {
  if (!Array.isArray(value)) {
    ...
    if (typeof entry !== "string" || entry.length === 0) {
      ...
      issues.push(
        issue(
          "error",
          "INVALID_IDENTIFIER",
          `Expected a URI-like identifier, received ${entry}`,
          `${path}[${index}]`,
        ),
      );
    }
...
export function validateDocumentShape(value: unknown): ValidationIssue[] {
  ...
  if (!isRecord(value)) {
    return [
      issue(
        "error",
        "INVALID_DOCUMENT",
        "Metamap document must be an object",
        "$",
      ),
    ];
  }
  ...
  if (value.schemaVersion !== METAMAP_SCHEMA_VERSION) {
    issues.push(
      issue(
        ...
        `Expected schemaVersion ${METAMAP_SCHEMA_VERSION}`,
        "$.schemaVersion",
      ),
    );
  }
    ...
    if (!Array.isArray(value.relationPacks)) {
      issues.push(
        issue(
          "error",
          "EXPECTED_ARRAY",
          "Relation packs must be an array",
          "$.relationPacks",
        ),
      );
    } else {
      for (const [index, pack] of value.relationPacks.entries()) {
        ...
        if (!isRecord(pack)) {
          issues.push(
            issue(
              "error",
              "INVALID_RELATION_PACK_REFERENCE",
              "Relation pack reference must be an object",
              path,
            ),
          );
          continue;
        ...
        if (!isReferenceIdentifier(pack.id)) {
          issues.push(
            issue(
              "error",
              "INVALID_IDENTIFIER",
              "Relation pack id must be URI-like",
              `${path}.id`,
            ),
          );
        }
        if (typeof pack.version !== "string" || pack.version.length === 0) {
          issues.push(
            issue(
              "error",
              "INVALID_RELATION_PACK_VERSION",
              "Relation pack version must be a non-empty string",
              `${path}.version`,
            ),
          );
        }
  ...
  if (!Array.isArray(value.entities)) {
    issues.push(
      issue("error", "EXPECTED_ARRAY", "Expected an array", "$.entities"),
    );
  } else {
    for (const [index, entity] of value.entities.entries()) {
      ...
      if (!isRecord(entity)) {
        issues.push(
          issue("error", "INVALID_ENTITY", "Entity must be an object", path),
        );
        continue;
      }
            ...
            if (!isRecord(locator) || !isReferenceIdentifier(locator.uri)) {
              issues.push(
                issue(
                  "error",
                  "INVALID_LOCATOR",
                  "Locator must have a URI-like uri",
                  locatorPath,
                ),
              );
            }
      ...
      if (entity.attributes !== undefined && !isJsonValue(entity.attributes)) {
        issues.push(
          issue(
            "error",
            "INVALID_ATTRIBUTES",
            "Attributes must contain JSON values",
            `${path}.attributes`,
          ),
        );
      }
  ...
  if (!Array.isArray(value.mappings)) {
    issues.push(
      issue("error", "EXPECTED_ARRAY", "Expected an array", "$.mappings"),
    );
  } else {
    for (const [index, mapping] of value.mappings.entries()) {
      ...
      if (!isRecord(mapping)) {
        issues.push(
          issue("error", "INVALID_MAPPING", "Mapping must be an object", path),
        );
        continue;
      }
      ...
      if (!isRelationIdentifier(mapping.relation)) {
        issues.push(
          issue(
            "error",
            "INVALID_RELATION_ID",
            "Relation must use namespace:name form",
            `${path}.relation`,
          ),
        );
      }
      ...
      if (
        typeof mapping.cardinality !== "string" ||
      ...
      if (
        typeof mapping.lossiness !== "string" ||
  ...
  if (!Array.isArray(value.authorities)) {
    issues.push(
      issue("error", "EXPECTED_ARRAY", "Expected an array", "$.authorities"),
    );
  } else {
    for (const [index, authority] of value.authorities.entries()) {
      ...
      for (const key of ["id", "concept", "source"] as const) {
        if (!isReferenceIdentifier(authority[key])) {
          issues.push(
            issue(
              "error",
              "INVALID_IDENTIFIER",
              `${key} must be URI-like`,
              `${path}.${key}`,
            ),
          );
        }
      ...
      if (authority.mode === "delegated") {
        if (!isReferenceIdentifier(authority.delegatedFrom)) {
          issues.push(
            issue(
              "error",
              "INVALID_DELEGATION_PARENT",
              "Delegated authority must reference its parent declaration",
              `${path}.delegatedFrom`,
            ),
          );
        }
        ...
        issues.push(
          issue(
            "error",
            "UNEXPECTED_DELEGATION_PARENT",
            "Only delegated authority may declare delegatedFrom",
            `${path}.delegatedFrom`,
          ),
        );
      }
...
function cardinalityMatches(mapping: StructuralMapping): boolean {
...
function factsCover(
  parent: AuthorityDeclaration,
  child: AuthorityDeclaration,
): boolean {
  if (parent.facts.includes("*")) return true;
  if (child.facts.includes("*")) return false;
  const parentFacts = new Set(parent.facts);
  return child.facts.every((fact) => parentFacts.has(fact));
}

function isDelegationAncestor(
  ancestorId: string,
  descendantId: string,
  authorities: ReadonlyMap<string, AuthorityDeclaration>,
): boolean {
  const visited = new Set<string>();
  let current = authorities.get(descendantId);
  while (current?.mode === "delegated" && !visited.has(current.id)) {
    if (current.delegatedFrom === ancestorId) return true;
...
function findDelegationCycle(
  authorities: readonly AuthorityDeclaration[],
): string[] | undefined {
  const byId = new Map(
...
function findCycle(
  ...
  for (const mapping of mappings) {
    if (mapping.relation !== relation) continue;
    for (const source of mapping.sources) {
      const targets = adjacency.get(source) ?? new Set<string>();
      for (const target of mapping.targets) targets.add(target);
...
export function validateDocumentSemantics(
    ...
    if (previous) {
      issues.push(
        issue(
          "error",
          "DUPLICATE_ID",
          `Identifier ${id} is already used at ${previous}`,
          path,
          id,
        ),
      );
  ...
  const entities = new Map<string, (typeof document.entities)[number]>();
    ...
    if (new Set(locatorUris).size !== locatorUris.length) {
      issues.push(
        issue(
          "warning",
          "DUPLICATE_LOCATOR",
          "Entity contains duplicate locators",
          `${path}.locators`,
          entity.id,
        ),
      );
  ...
  for (const pack of document.relationPacks ?? []) {
    if (!registry.hasPack(pack.id, pack.version)) {
      issues.push(
        issue(
          "error",
          "UNAVAILABLE_RELATION_PACK",
          `Relation pack ${pack.id}@${pack.version} is not registered`,
          "$.relationPacks",
          pack.id,
        ),
      );
    ...
    if (new Set(mapping.sources).size !== mapping.sources.length) {
      issues.push(
        issue(
          "error",
          "DUPLICATE_ENDPOINT",
          "Mapping sources must be unique",
          `${path}.sources`,
          mapping.id,
        ),
      );
    ...
    if (new Set(mapping.targets).size !== mapping.targets.length) {
      issues.push(
        issue(
          "error",
          "DUPLICATE_ENDPOINT",
          "Mapping targets must be unique",
          `${path}.targets`,
          mapping.id,
        ),
      );
    ...
    if (!definition) {
      issues.push(
        issue(
          "error",
          "UNKNOWN_RELATION",
          `Relation ${mapping.relation} is not registered`,
          `${path}.relation`,
          mapping.id,
        ),
      );
      ...
      if (
        ...
        issues.push(
          issue(
            ...
            `${mapping.relation} does not allow ${mapping.cardinality}`,
            `${path}.cardinality`,
            mapping.id,
          ),
        );
      ...
      if (!cardinalityMatches(mapping)) {
        issues.push(
          issue(
            ...
            `${mapping.cardinality} does not match ${mapping.sources.length} source(s) and ${mapping.targets.length} target(s)`,
            path,
            mapping.id,
          ),
        );
        ...
        if (!entity) {
          issues.push(
            issue(
              "error",
              "UNRESOLVED_ENDPOINT",
              `Mapping endpoint ${entityId} does not resolve`,
              `${path}.${side}[${endpointIndex}]`,
              mapping.id,
            ),
          );
          return;
        ...
        if (
          ...
          issues.push(
            issue(
              ...
              `${mapping.relation} does not allow ${entity.kind} on ${side}`,
              `${path}.${side}[${endpointIndex}]`,
              mapping.id,
            ),
          );
    ...
    if (!entities.has(authority.concept)) {
      issues.push(
        issue(
          "error",
          "UNRESOLVED_CONCEPT",
          `Authority concept ${authority.concept} does not resolve`,
          `${path}.concept`,
          authority.id,
        ),
      );
    ...
    if (!entities.has(authority.source)) {
      issues.push(
        issue(
          ...
          `Authority source ${authority.source} does not resolve`,
          `${path}.source`,
          authority.id,
        ),
      );
    ...
    if (new Set(authority.facts).size !== authority.facts.length) {
      issues.push(
        issue(
          "error",
          "DUPLICATE_FACT",
          "Authority facts must be unique",
          `${path}.facts`,
          authority.id,
        ),
      );
    ...
    if (authority.mode === "candidate") {
      issues.push(
        issue(
          ...
          `Authority for ${authority.concept} remains a candidate`,
          path,
          authority.id,
        ),
      );
  ...
  const canonical = document.authorities.filter(
    (authority) => authority.mode === "canonical",
  );
  const authoritiesById = new Map(
    document.authorities.map((authority) => [authority.id, authority]),
  );
      ...
      if (a.concept === b.concept && factsOverlap(a, b)) {
        issues.push(
          issue(
            "error",
            "AUTHORITY_CONFLICT",
            `Canonical authorities ${a.id} and ${b.id} overlap for ${a.concept}`,
            "$.authorities",
            a.concept,
          ),
        );
  ...
  const delegatedAuthorities = document.authorities.filter(
  ...
  for (const delegated of delegatedAuthorities) {
    ...
    if (!parent) {
      issues.push(
        issue(
          "error",
          "UNRESOLVED_DELEGATION_PARENT",
          `Delegated authority ${delegated.id} references missing parent ${delegated.delegatedFrom}`,
          "$.authorities",
          delegated.id,
        ),
      );
    ...
    if (parent.mode === "candidate") {
      issues.push(
        issue(
          "error",
          "INVALID_DELEGATION_PARENT",
          `Delegated authority ${delegated.id} cannot descend from candidate ${parent.id}`,
          "$.authorities",
          delegated.id,
        ),
      );
    ...
    if (parent.concept !== delegated.concept) {
      issues.push(
        issue(
          "error",
          "DELEGATION_CONCEPT_MISMATCH",
          `Delegation ${delegated.id} changes concept from ${parent.concept} to ${delegated.concept}`,
          "$.authorities",
          delegated.id,
        ),
      );
    ...
    if (!factsCover(parent, delegated)) {
      issues.push(
        issue(
          "error",
          "DELEGATION_FACT_WIDENING",
          `Delegation ${delegated.id} contains facts not owned by ${parent.id}`,
          "$.authorities",
          delegated.id,
        ),
      );
  ...
  for (let left = 0; left < delegatedAuthorities.length; left += 1) {
    for (
      let right = left + 1;
      right < delegatedAuthorities.length;
      right += 1
    ) {
      const a = delegatedAuthorities[left];
      const b = delegatedAuthorities[right];
      const sameBranch =
        isDelegationAncestor(a.id, b.id, authoritiesById) ||
        isDelegationAncestor(b.id, a.id, authoritiesById);
      if (a.concept === b.concept && factsOverlap(a, b) && !sameBranch) {
        issues.push(
          issue(
            "error",
            "DELEGATION_BRANCH_CONFLICT",
            `Delegated authorities ${a.id} and ${b.id} overlap without an ancestor relationship`,
            "$.authorities",
            a.concept,
          ),
        );
  ...
  if (delegationCycle) {
    issues.push(
      issue(
        "error",
        "DELEGATION_CYCLE",
        `Authority delegation contains a cycle: ${delegationCycle.join(" -> ")}`,
        "$.authorities",
        delegationCycle[0],
      ),
    );
  ...
  for (const definition of registry.all()) {
    if (definition.cyclePolicy !== "forbid") continue;
    ...
    if (cycle) {
      issues.push(
        issue(
          ...
          `${definition.id} contains a cycle: ${cycle.join(" -> ")}`,
          "$.mappings",
          definition.id,
        ),
      );
...
export function validateMetamapDocument(
  value: unknown,
  registry = new RelationRegistry(),
): ValidationResult {
  const shapeIssues = validateDocumentShape(value);
  const hasShapeErrors = shapeIssues.some(
    (entry) => entry.severity === "error",
  );
  const semanticIssues = hasShapeErrors
    ? []
    : validateDocumentSemantics(value as MetamapDocument, registry);
