...
import {
  collapseProjectedSlots,
  type MetamapProjection,
  type MetamapProjectionSpec,
  type PathTreeDelimiter,
  type PathTreeSpec,
...
const PATH_SEGMENT =
  /^(?:[A-Za-z0-9._~-]+|:[A-Za-z][A-Za-z0-9_]*|\*[A-Za-z][A-Za-z0-9_]*\??)$/;
const PATH_PARAM_TOKEN =
...
function isCatchAllParam(name: string): boolean {
...
export interface MetamapPathTree {
  ...
  id: string;
  digest: string;
  projection: { id: string; digest: string };
...
const pathTreeSchema = JSON.parse(
  readFileSync(
    new URL("../schemas/metamap-path-tree.schema.json", import.meta.url),
    "utf8",
  ),
) as AnySchema;
const pathTreeSchemaValidator = addFormats(
  new Ajv2020({ allErrors: true, strict: true }),
).compile(pathTreeSchema);
...
function schemaIssues(
  ...
  return (errors ?? []).map((error) =>
    issue("INVALID_PATH_TREE", error.message ?? "Path tree is invalid", {
...
function pathTreeConfig(spec: MetamapProjectionSpec): PathTreeSpec {
  return spec.pathTree ?? {};
}

function delimiterOf(spec: MetamapProjectionSpec): PathTreeDelimiter {
  return pathTreeConfig(spec).delimiter ?? "/";
}

function attributeOf(spec: MetamapProjectionSpec): string {
  return pathTreeConfig(spec).attribute ?? "path";
}

function pathAt(segments: string[], delimiter: PathTreeDelimiter): string {
  if (segments.length === 0) return delimiter === "/" ? "/" : delimiter;
  if (delimiter === "/") return `/${segments.join("/")}`;
...
function splitPath(
  ...
  delimiter: PathTreeDelimiter,
  subjectId: string,
): { segments?: string[]; issue?: ProjectionIssue } {
  const root = delimiter === "/" ? "/" : delimiter;
  if (path === root) return { segments: [] };
  const prefix = root;
  if (!path.startsWith(prefix)) {
    return {
      issue: issue(
        "PATH_TREE_INVALID_PATH",
        `Path ${path} must start with delimiter ${delimiter}`,
        { subjectId },
      ),
    };
  }
  if (delimiter === "/" && path.endsWith("/") && path !== "/") {
    return {
      issue: issue(
        "PATH_TREE_INVALID_PATH",
        `Path ${path} must not have a trailing delimiter`,
        { subjectId },
      ),
    };
  }
  ...
  if (rest.length === 0) {
    return {
      issue: issue(
        "PATH_TREE_INVALID_PATH",
        `Path ${path} is empty after its delimiter`,
        { subjectId },
      ),
    };
  }
  ...
  for (const segment of segments) {
    if (RESERVED_SEGMENTS.has(segment)) {
      return {
        issue: issue(
          "PATH_TREE_RESERVED_SEGMENT",
          `Path ${path} uses reserved segment ${segment}`,
          { subjectId },
        ),
      };
    }
    if (!PATH_SEGMENT.test(segment)) {
      return {
        issue: issue(
          "PATH_TREE_INVALID_PATH",
          `Path ${path} contains unsafe segment ${segment}`,
          { subjectId },
        ),
      };
    }
...
function occupantOf(
  subject: MetamapProjection["entries"][number]["subject"],
  slots: PathTreeOccupant["slots"],
): PathTreeOccupant {
  return {
    id: subject.id,
    kind: subject.kind,
...
function collectParams(node: PathTreeNode): string[] {
  const params = new Set<string>();
  function walk(current: PathTreeNode): void {
    for (const [key, value] of Object.entries(current)) {
      if (key === "_" || key === "$") continue;
      if (PATH_PARAM.test(key)) params.add(key);
      if (isRecord(value) && typeof value._ === "string") {
        walk(value as PathTreeNode);
      }
    }
  }
  walk(node);
...
function ensureChild(
  ...
  if (existing && typeof existing === "object" && "_" in existing) {
    return existing as PathTreeNode;
  }
  const child: PathTreeNode = { _: path };
...
export function compilePathTree(
  ...
  if (!spec.pathTree) {
    issues.push(
      issue(
        "PATH_TREE_SPEC_MISSING",
        `Projection ${spec.id} does not declare pathTree`,
        { subjectId: spec.id, path: "$.pathTree" },
      ),
    );
  }
  if (projection.spec.id !== spec.id) {
    issues.push(
      issue(
        "PATH_TREE_SPEC_MISMATCH",
        `Path tree spec ${spec.id} does not match projection spec ${projection.spec.id}`,
        { subjectId: spec.id },
      ),
    );
  }
  if (projection.spec.digest !== valueDigest(spec)) {
    issues.push(
      issue(
        ...
        `Path tree spec digest does not match projection ${projection.id}`,
        { subjectId: spec.id },
      ),
    );
  }
  ...
  const root: PathTreeNode = { _: pathAt([], delimiter) };
  ...
  for (const entry of entries) {
    ...
    if (typeof raw !== "string" || raw.length === 0) {
      issues.push(
        issue(
          "PATH_TREE_PATH_MISSING",
          `${entry.subject.id} has no string ${attribute} attribute`,
    ...
    if (canonical !== raw) {
      issues.push(
        issue(
          "PATH_TREE_INVALID_PATH",
          `${entry.subject.id} path ${raw} is not canonical; expected ${canonical}`,
          { subjectId: entry.subject.id },
        ),
      );
      continue;
    ...
    if (previous) {
      issues.push(
        issue(
          "PATH_TREE_COLLISION",
          `Path ${canonical} is claimed by both ${previous} and ${entry.subject.id}`,
          { subjectId: entry.subject.id },
        ),
      );
      continue;
    ...
    for (const [index, segment] of segments.entries()) {
      current = ensureChild(
        current,
        segment,
        pathAt(segments.slice(0, index + 1), delimiter),
      );
    }
    if (current.$) {
      issues.push(
        issue(
          "PATH_TREE_COLLISION",
          `Path ${canonical} already has occupant ${current.$.id}`,
          { subjectId: entry.subject.id },
        ),
      );
      continue;
  ...
  return {
    ...
    pathTree: deepFreeze({
      $schema:
        "https://raw.githubusercontent.com/roryscot/metamap/main/schemas/metamap-path-tree.schema.json",
      ...content,
...
export function validatePathTree(value: unknown): {
  valid: boolean;
  issues: ProjectionIssue[];
} {
  ...
  const pathTree = value as MetamapPathTree;
  ...
  if (digest !== expectedDigest) {
    issues.push(
      issue(
        "PATH_TREE_DIGEST_MISMATCH",
        `Path tree digest ${digest} does not match ${expectedDigest}`,
        { subjectId: id },
      ),
    );
  }
  if (id !== expectedId) {
    issues.push(
      issue(
        "PATH_TREE_ID_MISMATCH",
        `Path tree id ${id} does not match its content address`,
        { subjectId: id },
      ),
    );
  }
...
export function parsePathTree(value: unknown): MetamapPathTree {
  const result = validatePathTree(value);
  if (!result.valid || !isRecord(value)) {
    throw new Error(
      `Invalid Metamap path tree: ${result.issues
        .map((entry) => `${entry.path ?? "$"} ${entry.message}`)
        .join("; ")}`,
    );
  }
  return value as unknown as MetamapPathTree;
}
...
function isUnsafeParamValue(
  ...
  if (delimiter !== "/" && value.includes(delimiter)) return true;
  if (!isCatchAllParam(token) && value.includes("/")) return true;
  return false;
}

function collapseHydratedPath(
  path: string,
  delimiter: PathTreeDelimiter,
): string {
  const root = delimiter === "/" ? "/" : delimiter;
...
function substitutePath(
  ...
  params: Record<string, string>,
  delimiter: PathTreeDelimiter,
): { path?: string; missing?: string; unsafe?: string } {
  const tokens = [...new Set(path.match(PATH_PARAM_TOKEN) ?? [])].sort(
  ...
  for (const token of tokens) {
    ...
    if (isUnsafeParamValue(value, delimiter, token)) return { unsafe: token };
...
function hydrateNode(
  ...
  const hydrated: PathTreeNode = { _: node._ };
  ...
  if (node.$) hydrated.$ = structuredClone(node.$);
  for (const [key, value] of Object.entries(node)) {
    if (key === "_" || key === "$") continue;
    if (isRecord(value) && typeof value._ === "string") {
      hydrated[key] = hydrateNode(value as PathTreeNode, params, delimiter);
    }
  }
  return hydrated;
}
 ...
 * not mutated.
 */
export function hydratePathTree<T extends PathTreeNode>(
  tree: T,
  params: Record<string, string>,
  options: {
    delimiter?: PathTreeDelimiter;
  ...
  const required = (options.requiredParams ?? collectParams(tree)).filter(
    (name) => !isOptionalParam(name),
  );
  ...
  return hydrateNode(structuredClone(tree), params, delimiter) as T;
}
...
export function emitTypeScriptPathTree(
  ...
  return (
    `/* Generated by Metamap from ${pathTree.id}. Do not edit. */\n` +
