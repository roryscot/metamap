...
export function parseEvaluationContext(value: unknown): EvaluationContext {
  ...
  for (const [key, entry] of Object.entries(value)) {
    ...
    if (
      ...
      throw new Error(
        `Evaluation context ${key} must be a JSON primitive or primitive array`,
      );
    }
    context[key] = entry;
  }
...
function includesValue(container: ContextValue, expected: unknown): boolean {
...
export function evaluateContextExpression(
  ...
  if ("all" in expression) {
    return expression.all.every((child) =>
      evaluateContextExpression(child, context),
    );
  }
  ...
  if (expression.operator === "in" && "values" in expression) {
    ...
    return actualValues.some((entry) =>
      expression.values.some((expected) => valuesEqual(entry, expected)),
    );
  }
  if (!("values" in expression)) return false;
  const actualValues = Array.isArray(actual) ? actual : [actual];
  return actualValues.every((entry) =>
    expression.values.every((expected) => !valuesEqual(entry, expected)),
  );
}
...
export function requiredContextKeys(
  ...
  if ("all" in expression) {
    return new Set(
      expression.all.flatMap((child) => [...requiredContextKeys(child)]),
    );
  }
  if ("any" in expression) {
    return new Set(
      expression.any.flatMap((child) => [...requiredContextKeys(child)]),
    );
  }
  if ("not" in expression) return requiredContextKeys(expression.not);
...
export function inactiveReason(
  applicability: MappingApplicability | undefined,
  context: Readonly<EvaluationContext>,
): InactiveMappingReason | undefined {
  if (
    applicability?.when &&
