...
import type {
  NextjsAppRouterSourceConfig,
  NextjsTopologyExclusion,
} from "../config.js";
import {
  APPLICATION_TOPOLOGY_RELATION_PACK_ID,
  APPLICATION_TOPOLOGY_RELATION_PACK_VERSION,
...
interface NextFileObservation {
  ...
  loaders: LoaderObservation[];
  exclusion?: NextjsTopologyExclusion;
}
...
interface LoaderObservation {
  kind: "fetch" | "query" | "static-params" | "metadata";
  name: string;
  line: number;
  column: number;
}

const LOADER_CALL_KINDS = new Map<string, LoaderObservation["kind"]>([
  ["fetch", "fetch"],
  ["useQuery", "query"],
...
const LOADER_EXPORT_KINDS = new Map<string, LoaderObservation["kind"]>([
...
function segmentKind(segment: string): string {
  ...
  if (/^\(.+\)$/.test(segment)) return "group";
  if (/^\((?:\.|\.\.|\.\.\.)+\)/.test(segment)) return "intercepted";
  return "static";
}

function urlSegment(segment: string): string | undefined {
  const kind = segmentKind(segment);
  if (kind === "group" || kind === "parallel") return undefined;
  if (kind === "dynamic") return `:${segment.slice(1, -1)}`;
  if (kind === "catch-all") return `*${segment.slice(4, -1)}`;
  if (kind === "optional-catch-all") return `*${segment.slice(5, -2)}?`;
  if (kind === "intercepted")
    return segment.replace(/^\((?:\.|\.\.|\.\.\.)+\)/, "");
  return segment;
}

function routePath(directoryPath: string): string {
...
function dynamicParameters(directoryPath: string): Array<{
  name: string;
...
function sourceObservations(
  ...
  const imports = new Map<string, { exportedName: string; module: string }>();
  const loaders = new Map<string, LoaderObservation>();
  const loaderCalls = new Map(LOADER_CALL_KINDS);

  const observeLoader = (
    kind: LoaderObservation["kind"],
    name: string,
    node: ts.Node,
  ): void => {
  ...
  for (const statement of sourceFile.statements) {
    if (
      ...
      if (clause?.name) {
        ...
        if (module === "swr") loaderCalls.set(clause.name.text, "query");
      }
      ...
      if (bindings && ts.isNamedImports(bindings)) {
        for (const element of bindings.elements) {
          ...
          const loaderKind = LOADER_CALL_KINDS.get(
            element.propertyName?.text ?? element.name.text,
          );
          if (loaderKind) loaderCalls.set(element.name.text, loaderKind);
        }
      }
    }
    if (
      ...
      if (loaderKind) observeLoader(loaderKind, name, statement.name);
    }
    if (ts.isVariableStatement(statement) && hasExportModifier(statement)) {
      for (const declaration of statement.declarationList.declarations) {
        ...
        if (ts.isIdentifier(declaration.name)) {
          ...
          const loaderKind = LOADER_EXPORT_KINDS.get(name);
          if (loaderKind) observeLoader(loaderKind, name, declaration.name);
        }
      }
    }
  }
  ...
  const visit = (node: ts.Node): void => {
    if (ts.isJsxOpeningElement(node) || ts.isJsxSelfClosingElement(node)) {
      ...
      if (localName.endsWith("Provider")) {
        ...
        const column = position.character + 1;
        providers.set(
          `${imported?.module ?? path}:${localName}:${line}:${column}`,
          {
            localName,
    ...
    if (ts.isCallExpression(node)) {
      ...
      const loaderKind = name
        ...
        : undefined;
      if (name && loaderKind) observeLoader(loaderKind, name, expression);
    }
  ...
  return {
    ...
    providers: [...providers.values()].sort((left, right) =>
      `${left.line.toString().padStart(8, "0")}:${left.column.toString().padStart(8, "0")}:${left.module}:${left.localName}`.localeCompare(
        `${right.line.toString().padStart(8, "0")}:${right.column.toString().padStart(8, "0")}:${right.module}:${right.localName}`,
      ),
    ),
    loaders: [...loaders.values()].sort((left, right) =>
      `${left.line.toString().padStart(8, "0")}:${left.column.toString().padStart(8, "0")}:${left.kind}:${left.name}`.localeCompare(
        `${right.line.toString().padStart(8, "0")}:${right.column.toString().padStart(8, "0")}:${right.kind}:${right.name}`,
      ),
    ),
  };
}
...
async function collectFiles(
  config: NextjsAppRouterSourceConfig,
  ...
  const visit = async (directory: string): Promise<void> => {
    ...
    for (const entry of entries.sort((left, right) =>
      ...
      if (!SPECIAL_ROLES.has(role) || !extensions.has(extension)) continue;
      files.push({
        absolutePath,
...
async function observeFiles(
  ...
  observations: NextFileObservation[];
  diagnostics: AdapterDiagnostic[];
}> {
  const configured = new Map<string, NextjsTopologyExclusion>();
  ...
  const observations: NextFileObservation[] = [];
  for (const file of await collectFiles(config, context)) {
    ...
    const repositoryPath = normalized(
      relative(context.repositoryRoot, file.absolutePath),
    );
...
function addMapping(
  mappings: Map<string, StructuralMapping>,
  mapping: StructuralMapping,
): void {
  if (!mappings.has(mapping.id)) mappings.set(mapping.id, mapping);
}

function structureAttributes(
  config: NextjsAppRouterSourceConfig,
  observation: NextFileObservation,
 ...
 */
export class NextjsAppRouterAdapter implements MetamapAdapter<NextjsAppRouterSourceConfig> {
  readonly id = "nextjs-app-router" as const;
  ...
  async discover(
    ...
    const mappings = new Map<string, StructuralMapping>();
    ...
    const ensureSegment = (directoryPath: string): string => {
      ...
      if (name.startsWith("@")) {
        ...
        addMapping(mappings, {
          id: mappingId(
            ...
            config.id,
            "contains",
            `${directoryKey(parentPath)}:outlet:${name}`,
          ),
        ...
        addMapping(mappings, {
          id: mappingId(
            ...
            config.id,
            "contains",
            `${directoryKey(parentPath)}:segment:${key}`,
          ),
          relation: "topology:contains",
          sources: [parentId],
          targets: [id],
    ...
    const pages: Array<{ id: string; observation: NextFileObservation }> = [];
    ...
    for (const observation of observations) {
      ...
      if (observation.role === "route") {
        ...
        for (const method of observation.methods) {
          ...
          const routeId = entityId(
            context.namespace,
            config.id,
            "next-route",
            key,
          );
          const handlerId = entityId(
            context.namespace,
            config.id,
            "next-handler",
            key,
      ...
      entities.set(id, {
        ...
        attributes: structureAttributes(config, observation, {
          ...
          ...(kind === "topology:page"
            ? {
                dynamic:
                  dynamicParameters(observation.directoryPath).length > 0,
              }
            : {}),
        }),
        provenance,
      ...
      if (
        observation.client &&
        ["topology:page", "topology:container", "topology:boundary"].includes(
          kind,
        )
      ) {
        const hydrationId = entityId(
          context.namespace,
          config.id,
          "hydration-boundary",
          key,
    ...
    const containerRank = (entry: {
      directoryPath: string;
      role: string;
    }): string =>
      `${entry.directoryPath.split("/").filter(Boolean).length.toString().padStart(6, "0")}:${entry.directoryPath}:${entry.role === "layout" ? "0" : "1"}`;
    for (const page of pages) {
      const ancestors = containers
        ...
        .sort((left, right) =>
          containerRank(left).localeCompare(containerRank(right)),
        );
      ...
      if (parameters.length > 0) {
        const schemaId = entityId(
          context.namespace,
          config.id,
          "route-parameter-schema",
          directoryKey(page.observation.directoryPath),
        );
