...
type TopologyCardinality =
  "exactly-one" | "zero-or-one" | "one-or-more" | "many";

function entityMatches(
  entity: MetamapDocument["entities"][number],
  kinds: readonly string[],
  attributes: JsonObject | undefined,
): boolean {
  if (!kinds.includes(entity.kind)) return false;
  return Object.entries(attributes ?? {}).every(
    ([key, value]) =>
      JSON.stringify(entity.attributes?.[key]) === JSON.stringify(value),
  );
}

function topologyExcluded(
  entity: MetamapDocument["entities"][number],
  relation?: string,
): boolean {
  if (entity.attributes?.topologyDisposition !== "excluded") return false;
  const relations = entity.attributes.exclusionRelations;
  if (relation === undefined || relations === undefined) return true;
  return Array.isArray(relations) && relations.includes(relation);
}

function validateTopologyExclusion(
  constraint: ViabilityConstraint,
  entity: MetamapDocument["entities"][number],
): ViabilityIssue[] {
  if (!topologyExcluded(entity)) return [];
  if (
    typeof entity.attributes?.exclusionReason !== "string" ||
    ...
    return [
      issue(
        constraint,
        "INVALID_TOPOLOGY_EXCLUSION",
        `${entity.id} is excluded without a non-empty exclusionReason and exclusionOwner`,
        entity.id,
      ),
    ];
  }
  ...
  if (
    ...
    (!Array.isArray(relations) ||
      relations.length === 0 ||
      !relations.every(
        (entry) => typeof entry === "string" && isRelationIdentifier(entry),
      ))
  ) {
    return [
      issue(
        constraint,
        "INVALID_TOPOLOGY_EXCLUSION",
        `${entity.id} has invalid exclusionRelations`,
        entity.id,
      ),
    ];
  }
...
function topologyMappings(
  context: ConstraintEvaluationContext,
  subject: string,
  relation: string,
  direction: "source" | "target",
) {
  return activeMappings(context).filter(
    (mapping) =>
...
function topologyCounterparts(
  mappings: ReturnType<typeof topologyMappings>,
  direction: "source" | "target",
): string[] {
  return [
    ...new Set(
      mappings.flatMap((mapping) =>
        direction === "source" ? mapping.targets : mapping.sources,
      ),
    ),
  ];
}

const topologyRelationTotality: ConstraintEvaluator = (constraint, context) => {
  const allowed = [
    "subjectKinds",
    "subjectAttributes",
    "relation",
  ...
  const subjectAttributes = parameterObject(
    constraint.parameters,
    "subjectAttributes",
  );
  const relation = parameterString(constraint.parameters, "relation");
  const direction = parameterString(constraint.parameters, "direction");
  const cardinality = parameterString(constraint.parameters, "cardinality") as
    TopologyCardinality | undefined;
  const counterpartKinds = parameterStrings(
    constraint.parameters,
    "counterpartKinds",
  );
  const allowExcluded =
    parameterBoolean(constraint.parameters, "allowExcluded") ?? false;
  if (
    !subjectKinds ||
    ...
    !cardinality ||
    !["exactly-one", "zero-or-one", "one-or-more", "many"].includes(
      cardinality,
    ) ||
    ...
    return [
      issue(
        constraint,
        "INVALID_CONSTRAINT_PARAMETERS",
        `${constraint.id} requires subjectKinds, a relation, source|target direction, and a supported cardinality`,
      ),
    ];
  }

  const kinds = new Map(
    context.document.entities.map((entry) => [entry.id, entry.kind]),
  );
  const subjects = context.document.entities.filter((entry) =>
    entityMatches(entry, subjectKinds, subjectAttributes),
  );
  if (subjects.length === 0) {
    return [
      issue(
        constraint,
        "EMPTY_TOPOLOGY_SELECTION",
        `${constraint.id} selects no topology entities`,
      ),
    ];
  }
  ...
  return subjects.flatMap((subject) => {
    ...
    const matches = topologyMappings(
      context,
      subject.id,
      relation,
      direction as "source" | "target",
    );
    if (topologyExcluded(subject, relation)) {
      if (!allowExcluded) {
        return [
          ...
          issue(
            constraint,
            "TOPOLOGY_EXCLUSION_NOT_ALLOWED",
            `${subject.id} is excluded, but ${constraint.id} does not allow exclusions`,
            subject.id,
          ),
        ];
      }
      return matches.length === 0
        ...
        : [
            ...
            issue(
              constraint,
              "EXCLUDED_TOPOLOGY_SUBJECT_MAPPED",
              `${subject.id} is both excluded and mapped through ${relation}`,
              subject.id,
            ),
          ];
    }
    const counterparts = topologyCounterparts(
      matches,
      direction as "source" | "target",
    );
    ...
    const missing =
      (cardinality === "exactly-one" || cardinality === "one-or-more") &&
      count === 0;
    const ambiguous =
      (cardinality === "exactly-one" || cardinality === "zero-or-one") &&
      count > 1;
    return [
      ...exclusionIssues,
      ...(wrongKinds.length > 0
        ? [
            issue(
              constraint,
              "TOPOLOGY_COUNTERPART_KIND_MISMATCH",
              `${subject.id} resolves through ${relation} to disallowed kind(s): ${wrongKinds.join(", ")}`,
              subject.id,
            ),
          ]
        : []),
      ...(missing
        ? [
            issue(
              constraint,
              "TOPOLOGY_REQUIRED_RELATION_MISSING",
              `${subject.id} has no active ${relation} counterpart`,
              subject.id,
            ),
          ]
        : []),
      ...(ambiguous
        ? [
            issue(
              constraint,
              "TOPOLOGY_RELATION_AMBIGUOUS",
              `${subject.id} resolves through ${relation} to ${count} counterparts`,
              subject.id,
            ),
          ]
        : []),
...
const topologyReachable: ConstraintEvaluator = (constraint, context) => {
  const unexpected = unexpectedParameters(constraint, [
    "root",
    "relation",
    "subjectKinds",
  ...
  if (
    ...
    return [
      issue(
        constraint,
        "INVALID_CONSTRAINT_PARAMETERS",
        `${constraint.id} requires a known root, relation, and subjectKinds`,
      ),
    ];
  }
  const outgoing = new Map<string, string[]>();
  for (const mapping of activeMappings(context).filter(
  ...
  const selected = context.document.entities.filter((entry) =>
    entityMatches(entry, subjectKinds, subjectAttributes),
  );
  ...
  return selected.flatMap((entity) =>
    visited.has(entity.id)
      ...
      : [
          issue(
            constraint,
            "TOPOLOGY_ENTITY_UNREACHABLE",
            `${entity.id} is not reachable from ${root} through ${relation}`,
            entity.id,
          ),
        ],
  );
...
const topologyContextSatisfied: ConstraintEvaluator = (constraint, context) => {
  const unexpected = unexpectedParameters(constraint, [
    "subjectKinds",
    "subjectAttributes",
    "requiresRelation",
  ...
  const requiresRelation = parameterString(
    constraint.parameters,
    "requiresRelation",
  );
  const providesRelation = parameterString(
    constraint.parameters,
    "providesRelation",
  );
  const renderedInRelation = parameterString(
    constraint.parameters,
    "renderedInRelation",
  );
  const parentRelation = parameterString(
    constraint.parameters,
    "parentRelation",
  );
  const hostsProviderRelation = parameterString(
    constraint.parameters,
    "hostsProviderRelation",
  );
  const relations = [
  ...
  if (
    ...
    return [
      issue(
        constraint,
        "INVALID_CONSTRAINT_PARAMETERS",
        `${constraint.id} requires subjectKinds and five valid topology relations`,
      ),
    ];
  }

  const active = activeMappings(context);
  const outgoingTargets = (source: string, relation: string): string[] => [
    ...new Set(
      active
        .filter(
          (mapping) =>
            mapping.relation === relation && mapping.sources.includes(source),
        )
        .flatMap((mapping) => mapping.targets),
    ),
  ];
  const incomingSources = (target: string, relation: string): string[] => [
    ...new Set(
      active
        .filter(
          (mapping) =>
            mapping.relation === relation && mapping.targets.includes(target),
        )
        .flatMap((mapping) => mapping.sources),
    ),
  ];
  const selected = context.document.entities.filter(
    (entry) =>
      entityMatches(entry, subjectKinds, subjectAttributes) &&
  ...
  return selected.flatMap((subject) => {
    ...
    return required.flatMap((requiredContext) => {
      ...
      while (level.length > 0) {
        ...
        for (const scope of level) {
          for (const contextId of outgoingTargets(scope, providesRelation!)) {
            if (contextId === requiredContext) providers.add(scope);
          }
          for (const provider of outgoingTargets(
            scope,
            hostsProviderRelation!,
          )) {
            if (
              outgoingTargets(provider, providesRelation!).includes(
                requiredContext,
              )
            ) {
        ...
        if (providers.size > 1) {
          return [
            issue(
              constraint,
              "TOPOLOGY_CONTEXT_PROVIDER_AMBIGUOUS",
              `${subject.id} resolves ${requiredContext} to ${providers.size} providers at the same scope depth`,
              subject.id,
            ),
          ];
        }
      ...
      return [
        issue(
          constraint,
          "TOPOLOGY_CONTEXT_PROVIDER_MISSING",
          `${subject.id} requires ${requiredContext}, but no provider dominates its rendering scope`,
          subject.id,
        ),
      ];
    });
