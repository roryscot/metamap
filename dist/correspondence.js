...
function attribute(entity, name) {
    return entity.attributes?.[name];
}
function stringAttribute(entity, name) {
    const value = attribute(entity, name);
    return typeof value === "string" ? value : undefined;
}
function booleanAttribute(entity, name) {
    const value = attribute(entity, name);
    return typeof value === "boolean" ? value : undefined;
}
function structureKindMatches(entity, reference) {
...
function resolveStructure(entities, reference) {
    return entities.filter((entity) => stringAttribute(entity, "sourceId") === reference.sourceId &&
        stringAttribute(entity, "name") === reference.name &&
        structureKindMatches(entity, reference));
}
function childrenOf(entities, structureId) {
    return entities.filter((entity) => stringAttribute(entity, "structureId") === structureId);
}
...
function typesCompatible(source, target) {
    ...
    if (source === target)
        return true;
    if ((source.startsWith("reference:") || source.startsWith("enum:")) &&
        (target.startsWith("reference:") || target.startsWith("enum:"))) {
        return symbolicType(source) === symbolicType(target);
    }
    return false;
}
function factMatches(fact, source, target) {
    if (fact === "type") {
        return typesCompatible(stringAttribute(source, "dataType"), stringAttribute(target, "dataType"));
    }
    if (fact === "cardinality") {
        return (stringAttribute(source, "cardinality") ===
            stringAttribute(target, "cardinality"));
    }
    if (fact === "presence") {
        return (booleanAttribute(source, "presenceRequired") ===
            booleanAttribute(target, "presenceRequired"));
    }
    return (booleanAttribute(source, "nullable") ===
        booleanAttribute(target, "nullable"));
}
function factDescription(fact, entity) {
    if (fact === "type")
        return stringAttribute(entity, "dataType") ?? "unknown";
    if (fact === "cardinality") {
...
function mapping(id, relation, sourceId, targetId, provenance, attributes, transform) {
    return {
        id,
        relation,
        sources: [sourceId],
...
function checkCoverage(side, fields, mapped, ignoredNames, required, correspondenceId, issues) {
...
function processEnum(config, source, target, allEntities, namespace, provenance, mappings, issues) {
    ...
    for (const [name, sourceValue] of sourceValues) {
        ...
        if (targetValue) {
            mappings.push(mapping(makeUrn(namespace, "mapping", `${config.id}:enum-value:${name}`), config.relation ?? "core:derives_from", sourceValue.id, targetValue.id, provenance, { correspondenceId: config.id, value: name }));
        }
        else {
...
export function materializeCorrespondences(namespace, documentId, discovered, configurations, configRevision) {
    const provenance = {
        status: "declared",
    ...
    const authorities = [];
    const issues = [];
    const configuredStructureIds = new Set();
    for (const config of configurations) {
        ...
        mappings.push(mapping(makeUrn(namespace, "mapping", `${config.id}:structure`), config.relation ?? "core:derives_from", source.id, target.id, provenance, { correspondenceId: config.id }));
        if (config.authority) {
            authorities.push({
        ...
        if (config.source.kind === "enum" && config.target.kind === "enum") {
            processEnum(config, source, target, discovered.entities, namespace, provenance, mappings, issues);
        }
        if (!config.fields)
            continue;
        ...
        for (const pair of config.fields.pairs) {
            ...
            for (const fact of compared) {
                if (factMatches(fact, sourceField, targetField))
                    continue;
            ...
            mappings.push(mapping(makeUrn(namespace, "mapping", `${config.id}:field:${pair.source}:${pair.target}`), config.relation ?? "core:derives_from", sourceField.id, targetField.id, provenance, {
                correspondenceId: config.id,
        ...
        checkCoverage("source", sourceFields, mappedSource, config.fields.ignoreSource ?? [], config.fields.requireSourceCoverage ?? false, config.id, issues);
        checkCoverage("target", targetFields, mappedTarget, config.fields.ignoreTarget ?? [], config.fields.requireTargetCoverage ?? false, config.id, issues);
    }
    return {
        issues,
