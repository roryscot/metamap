...
export class MetamapValidationError extends Error {
    issues;
    constructor(issues) {
        super(`Metamap validation failed with ${issues.filter((entry) => entry.severity === "error").length} error(s)`);
        this.issues = issues;
...
export class MetamapGraph {
    ...
    mappingsByEntity = new Map();
    authoritiesByConcept = new Map();
    ...
    constructor(document, options = {}) {
        ...
        for (const authority of document.authorities) {
            ...
            if (authority.mode === "delegated") {
                const children = this.delegationsByParent.get(authority.delegatedFrom) ?? [];
    ...
    mappingsFor(entityId, options = {}) {
        ...
        return (this.mappingsByEntity.get(entityId) ?? []).filter((mapping) => {
            if (relationFilter && !relationFilter.has(mapping.relation))
                return false;
            const symmetric = this.registry.get(mapping.relation)?.symmetric === true;
            if (direction === "both" || symmetric)
                return true;
            if (direction === "outgoing")
                return mapping.sources.includes(entityId);
    ...
    neighbors(entityId, options = {}) {
        ...
        for (const mapping of this.mappingsFor(entityId, options)) {
            for (const linkedId of this.linkedEntityIds(mapping, entityId, direction)) {
    ...
    trace(entityId, options = {}) {
        ...
        const visits = [{ entity: start, depth: 0 }];
    ...
    resolveAuthority(conceptId, fact) {
        const applies = (authority) => authority.facts.includes("*") || authority.facts.includes(fact);
        const applicable = (this.authoritiesByConcept.get(conceptId) ?? []).filter(applies);
        const canonical = applicable.filter((authority) => authority.mode === "canonical");
        const delegated = applicable.filter((authority) => authority.mode === "delegated");
        const candidates = applicable.filter((authority) => authority.mode === "candidate");
        if (canonical.length === 1) {
            const chain = [canonical[0]];
            const visited = new Set([canonical[0].id]);
            let current = canonical[0];
            while (true) {
                const children = (this.delegationsByParent.get(current.id) ?? []).filter(applies);
    ...
    linkedEntityIds(mapping, entityId, direction) {
