...
export function isReferenceIdentifier(value) {
    return typeof value === "string" && IDENTIFIER_PATTERN.test(value);
}
export function isRelationIdentifier(value) {
    return typeof value === "string" && RELATION_PATTERN.test(value);
}
export function makeUrn(namespace, kind, value) {
    const encoded = encodeURIComponent(value);
    return `urn:${namespace}:${kind}:${encoded}`;
}
/** Convert a repository-relative path or glob into a resolvable locator URI. */
export function makeRepositoryUri(rawPath, repository = "workspace") {
...
export function hasAmbiguousPathLanguage(path) {
    return /\s+or\s+/i.test(path);
}
export function isPathPattern(path) {
    return /[*?{}[\]]/.test(path);
}
